...
# LTE
MAKEFILE=../../../$OUTDIR/device-common-vendor-blobs-lte.mk
...
LINEEND=" \\"
COUNT=`wc -l proprietary-files-lte.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' proprietary-files-lte.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../$DEVICE/proprietary-files-lte.txt`; do
    COUNT=`expr $COUNT - 1`
    if [ $COUNT = "0" ]; then
        LINEEND=""
    fi
